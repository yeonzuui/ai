...
SELECT * 
    ...
    SELECT EMPNO, ENAME, JOB, MGR, E.DEPTNO, DNAME, LOC -- 공통된 필드는 명확하게 테이블 지정
        FROM EMP "E" , DEPT "D"
        WHERE E.DEPTNO = D.DEPTNO;
    -- EX. SAL이 2000이상인 직원에 대해 이름, 직책, 급여, 부서명, 근무지
    ...
    -- EX. 82년도에 입사한 10번, 20번 부서 직원의 이름, 급여, 근무지 출력(급여순)
...
-- ★ 2. NON-EQUI JOIN
    SELECT * FROM EMP 
        WHERE ENAME = 'SCOTT'; --SCOTT 직원의 SAL등급은? 
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO MANAGER_NUM, MANAGER.ENAME MANAGER_NAME
    ...
    SELECT W.ENAME, M.ENAME "MANAGER"
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO;
    -- EX. "SMITH의 상사는 FORD" 출력
    SELECT W.ENAME ||'의 상사는 ' || M.ENAME "MESSAGE"
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO;
    -- 탄탄 전 준비작업(20번 부서 - DALLAS, 50번 부서 생성 - DALLAS)
...
SELECT W.ENAME, W.SAL, D.DEPTNO, DNAME, LOC, NVL(M.ENAME,'없음') "MANAGER"
    FROM EMP W, EMP M, DEPT D
    WHERE W.MGR=M.EMPNO(+)
        AND W.DEPTNO=D.DEPTNO;
...
SELECT W.ENAME, W.SAL, (W.SAL+NVL(W.COMM, 0)*12)"ANNUALSAL", GRADE, D.DNAME, M.ENAME "MANAGER" 
    FROM EMP W, EMP M, SALGRADE, DEPT D
    WHERE W.MGR=M.EMPNO
        AND W.SAL BETWEEN LOSAL AND HISAL
        AND W.DEPTNO=D.DEPTNO;
...
SELECT W.ENAME, M.ENAME "MANAGER", MOFM.ENAME "UPPERMANAGER"
    FROM EMP W, EMP M, EMP MOFM
    WHERE W.MGR=M.EMPNO
        AND M.MGR=MOFM.EMPNO;
