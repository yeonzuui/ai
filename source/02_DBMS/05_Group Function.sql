...
SELECT COUNT(*) FROM EMP; -- 테이블의 행 개수
    ...
    SELECT
        MIN(HIREDATE) FIRST, MAX(HIREDATE) LAST 
        FROM EMP;
    -- 탄탄EX2. 결과) FIRST                LAST
    ...
    SELECT
        ...
        FROM EMP;
    -- 탄탄EX3. 결과) 80년12월17일최초입사: 18,123일째 83년01월12일최근입사:16,123일째
    SELECT
    ...
    SELECT COUNT(*) --추천
        ...
        WHERE DEPTNO = 10 
            AND COMM > 0; 
    SELECT * FROM EMP WHERE DEPTNO = 10; --확인
    SELECT COUNT(COMM) 
        FROM EMP 
        WHERE DEPTNO = 10; --COMM이 0인 행을 포함

-- ★ 2. GROUP BY절: 그룹함수를 쓰되, 어떤 칼럼 기준으로 그룹함수를 적용
    -- EX. 부서번호별 최대급여, 급여평균, 급여표준편차 (부서번호순 정렬)
    SELECT DEPTNO NO, MAX(SAL) MAXSAL, 
           ROUND(AVG(SAL))     AVGSAL, 
           ROUND(STDDEV(SAL))  STDSAL 
        FROM EMP
        GROUP BY DEPTNO -- GROUP BY 절은 필드의 별칭 사용 불가
        ORDER BY NO;    -- ORDER BY 절은 필드의 별칭 사용 가능
    -- 부서명별 최대급여, 평균급여 (평균급여순 정렬)
    SELECT DNAME "DEPTNAME", 
            ROUND(MAX(SAL)) "MAXSAL", 
            ROUND(AVG(SAL)) "AVGSAL"
        FROM EMP E, DEPT D
    ...
    -- SAL이 5000미만인 사원에 대해서만 부서번호별 사원수, 최대급여, 최소급여, 평균급여
        -- (부서번호 순 정렬)
    SELECT DEPTNO, COUNT(*) COUNTEMP, 
            ROUND(MAX(SAL)) MAXSAL, 
            ROUND(MIN(SAL)) MINSAL, 
            ROUND(AVG(SAL)) AVGSAL
        FROM EMP
        WHERE SAL < 5000
        GROUP BY DEPTNO
...
--      (필드의 조건을 사용하여 제한할 때는 WHERE절)
    ...
    SELECT DEPTNO , COUNT(*) COUNTEMP, 
        ...
        FROM EMP
        WHERE SAL < 5000
        GROUP BY DEPTNO         -- GROUP BY 앞뒤(보통 뒤)에 HAVING절
        HAVING AVG(SAL) >= 1800 -- HAVING 절에 필드 별칭 사용 불가
        ORDER BY DEPTNO;
    -- EX. SAL이 5000미만인 사원에 대해 부서이름별 급여평균과 표준편차를 소수점 2자리까지 출력
        -- (단, 평균급여가 1800이상인 부서에 대해 부서명순으로 정렬)
    SELECT DNAME,
           ROUND(AVG(SAL), 2) "AVGSAL",
           ROUND(STDDEV(SAL), 2) "STDSAL"
        FROM EMP E, DEPT D
        WHERE E.DEPTNO = D.DEPTNO
            AND SAL < 5000
        GROUP BY DNAME
...
SELECT DEPTNO, SUM(SAL) 
    FROM EMP 
    GROUP BY ROLLUP(DEPTNO); -- EMP 테이블 전체의 SAL 합
SELECT DEPTNO, JOB, SUM(SAL) 
    FROM EMP 
...
SELECT MAX(SAL)-MIN(SAL) "MAX-" FROM EMP;

-- 4. 각 부서별로 인원수, 급여 평균, 최저 급여, 최고 급여, 급여의 합을 출력(급여의 합이 많은 순으로)
SELECT DNAME, E.DEPTNO, COUNT(*), ROUND(AVG(SAL)) AVG , MIN(SAL) MIN, MAX(SAL) MAX, SUM(SAL) SUM
    FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO
...
SELECT JOB, DEPTNO, COUNT(*), ROUND(AVG(SAL)) "AVG(SAL)", SUM(SAL)
    FROM EMP
    GROUP BY JOB, DEPTNO
    ORDER BY JOB, DEPTNO;
...
SELECT JOB, ROUND(AVG(SAL)) "SAL_AVERAGE" , SUM(SAL) "SAL_SUM"
    FROM EMP
    GROUP BY JOB
...
--  1983     1
--  total    14
SELECT NVL(TO_CHAR(HIREDATE, 'YYYY'), 'total') YEAR, COUNT(*) COUNT
    FROM EMP
    GROUP BY ROLLUP(TO_CHAR(HIREDATE, 'YYYY'));
-- EXTRACT(숫자로 추출) 사용 - TO_CHAR(문자로 추출) 
