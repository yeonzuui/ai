...
<body>
	...
	<%
		...
		// 자바에서 request 객체에 setAttribute한 내용을 jsp로 전달
		// request 객체를 유지한 상태에서 페이지 변경 (forward)
		RequestDispatcher dispatcher 
			= request.getRequestDispatcher("ex2_requestScope.jsp?id=aaa&pw=111"); // forward 할 jsp파일
		dispatcher.forward(request, response); //페이지 넘어감
	%>
	<h2><%=request.getAttribute("requestAtt") %></h2>
	<h2>${requestScope.requestAtt }</h2>
	<h2>${requestAtt }</h2> <!--requestScope. 생략가능, 속성만 써도 돼-->
</body>
</html>